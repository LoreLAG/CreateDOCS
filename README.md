...
* **Automated Resource Architecture:** Upon execution, the system maintains all necessary master databases and Excel templates within a localized, hidden directory named "Useful files for FC-FMEA-CP" to ensure environment integrity.
...
* **Useful files for FC-FMEA-CP/** (Hidden): Secure repository for master databases (MASTERS-FMEA.xlsx, MASTERS-CP.xlsx) and standardized templates.
...
* **ReadMASTERFMEA.py / ReadMASTERCP.py:** Backend utilities for database structural formatting and cleaning.

## 5. User Manual

### 5.1 Initialization
Execute the main application script. On the initial run, the system will verify or extract the required resource files into the "Useful files for FC-FMEA-CP" directory. The interface will display the active paths for templates and databases in the top configuration panel.
...
4. **Visibility Control:** Use the "Generate customer version" checkbox to flag phases that must be included in the customer-facing version of the documents.
...
When initiating Control Plan generation, if the system identifies multiple functional variants in the database for a specific phase, the "Global Control Plan Composer" will activate:
...
4. **Confirmation:** Use the Enter key or the confirm button to commit the composition.

### 5.4 Document Finalization and Export
1. **Project Metadata:** Enter the Product Name and the current Revision index.
2. **Export Scope:** Select the specific document required or use the comprehensive "ALL" command to generate a unified Master Workbook.
...
4. **Output:** Finalized documents are exported to a structured directory named "[Product Name] Documents" located in the application root.
